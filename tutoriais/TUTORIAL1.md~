...
Para iniciar o SDL, crie exceções para se organizar melhor e saber os erros dentro do código. A função `SDL_INIT` iniciará o SDL, e retornará -1 caso ocorra algum erro. Inicializando o SDL:
...
Caso não ocorra erro na função, cria-se a window através da função `SDL_CreateWindow` e checa se o ponteiro `window` a contem:
